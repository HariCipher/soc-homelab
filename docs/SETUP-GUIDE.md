...
```powershell
PS> New-ADUser -Name "svc_backup_adm" -SamAccountName "svc_backup_adm" `
      -Description "DO NOT USE - honeytoken" -Enabled $true `
      -AccountPassword (Read-Host -AsSecureString "password")
PS> Set-ADUser svc_backup_adm -ServicePrincipalNames @{Add="MSSQLSvc/fake.homelab.lan:1433"}
```
Add a decoy share with object-access auditing enabled, then write a Wazuh rule that
alerts on **any** authentication attempt or file access involving either. Nobody has
a legitimate reason to touch them, so every hit is real signal.

### DONE Phase 6 exit
Touch the honeytoken yourself; the alert fires. screenshot `evidence/deception/`.

---

## Phase 7 — n8n SOAR

Install n8n on the host, then build three workflows:
...
Trigger from the Wazuh API or active response, not by polling files. Start
notify-only; add containment once you trust the false-positive rate.

### DONE Phase 7 exit
One alert travels Wazuh → n8n → action with zero manual steps.
screenshot `evidence/automation/` — workflow canvas + a successful run.
...
```bash
$ bash scripts/verify/<phase>.sh          # must exit 0
# screenshot into evidence/<area>/YYYY-MM-DD-what.png
# update STATUS.md — only now may a row become DONE
$ git add -A && git commit -m "Phase N: <what was verified>"
```
